...
// Copyright (c) 2011-2012 YounetCoin Developers
...
void ThreadIRCSeed()
{
    ...
    RenameThread("YounetCoin-ircseed");
...
void ThreadIRCSeed2()
{
    ...
    while (true)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #YounetCoinTEST0\r");
            Send(hSocket, "WHO #YounetCoinTEST0\r");
        } else {
            // randomly join #YounetCoin00-#YounetCoin99
            // network is now over 3k peers , get them to join 50 random channels!
            //            channel_number = 0; 
            int channel_number = 0; //GetRandInt(50);

            Send(hSocket, strprintf("JOIN #YounetCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #YounetCoin%02d\r", channel_number).c_str());
        }
