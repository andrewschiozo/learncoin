...
uint256 hashGenesisBlock("0x06265ca8f4fef71ec717fea5883c9144cba7274b6426b29256f10707985633b1");
...
const string strMessageMagic = "YounetCoin Signed Message:\n";
...
{
    // YounetCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
{
    // YounetCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "ync genesis block";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xd2, 0xc3, 0xa1, 0xec };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("YounetCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("YounetCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static YounetCoinMiner(CWallet *pwallet)
{
    printf("YounetCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("YounetCoin-miner");
    ...
    try { loop {
        ...
        printf("Running YounetCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("YounetCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&YounetCoinMiner, pwallet));
}
